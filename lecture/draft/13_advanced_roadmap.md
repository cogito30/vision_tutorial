...
1. 모든 것을 하나로 해결하는 거대 파운데이션 모델 (VFM)
2. 2D 평면을 넘어 공간을 완벽하게 재구성하는 3D 비전 (3DGS)
...
- **Hands-on Action**: PyTorch Hub를 통한 DINOv2 초간단 로드 및 특징 추출 테스트
```python
import torch
...
1. **\[VFM 응용\] SAM 2 기반 오토 라벨링 파이프라인 툴 제작**:
- 사용자가 웹(Streamlit 등)에서 이미지를 업로드하고 점을 찍으면, SAM 2가 마스크를 따고 이를 YOLO 학습용 텍스트 데이터 포맷으로 자동 변환하여 다운로드해 주는 사내용 MLOps 툴을 만들어 보세요.

2. **\[3D 응용\] 3DGS 디지털 트윈 웹 뷰어**:
- 스마트폰으로 특정 공간(내 방, 카페 등)을 촬영한 후 `nerfstudio`로 3DGS 모델을 학습시킵니다. 그리고 WebGL을 활용해 웹 브라우저에서 사용자가 마우스로 3D 공간을 자유롭게 돌아다닐 수 있는 페이지를 구현하세요.

3. **\[공간 지능\] Depth Anything을 활용한 로봇 장애물 감지**:
